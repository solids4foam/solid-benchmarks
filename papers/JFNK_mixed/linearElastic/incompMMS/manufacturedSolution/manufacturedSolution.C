...
void Foam::manufacturedSolution::calcBodyForces() const
{
    ...
    forAll(bodyForcesI, cellI)
    {
	  ...
	  21*mu_*pi*pi*ax_*Foam::sin(4*pi*x)*Foam::sin(2*pi*y)*Foam::sin(pi*z);

        bodyForcesI[cellI][vector::Y] =
	  21*mu_*pi*pi*ay_*Foam::sin(4*pi*x)*Foam::sin(2*pi*y)*Foam::sin(pi*z);
	  ...
	  (
	   Foam::cos(pi*z)
	   *(
	     4*ax_*Foam::cos(4*pi*x)*Foam::sin(2*pi*y)
	     + 2*ay_*Foam::sin(4*pi*x)*Foam::cos(2*pi*y)
	     )
	   );
    }
...
)
{
    ...
    sigma.xx() =
      lambda*(4*ax_*pi*Foam::cos(4*pi*point.x())*Foam::sin(2*pi*point.y())*Foam::sin(pi*point.z())
	      ...
	      - pi*Foam::sin(pi*point.z())
	      *(
		4*ax_*Foam::cos(4*pi*point.x())*Foam::sin(2*pi*point.y())
		+ 2*ay_*Foam::sin(4*pi*point.x())*Foam::cos(2*pi*point.y())
		)
	      )
    ...
    sigma.yz() =
      Foam::cos(pi*point.z())
      *(
	8*ax_*mu*pi*Foam::cos(4*pi*point.x())*Foam::cos(2*pi*point.y())
	- 3*ay_*mu*pi*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y())
	);
    ...
    sigma.xz() =
      Foam::cos(pi*point.z())
      *(
	-15*ax_*mu*pi*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y())
	+ 8*ay_*mu*pi*Foam::cos(4*pi*point.x())*Foam::cos(2*pi*point.y())
	);
...
)
{
    ...
    epsilon.yz() =
      (pi*Foam::cos(pi*point.z())/2)
       *(
            8*ax_*Foam::cos(4*pi*point.x())*Foam::cos(2*pi*point.y())
          - 3*ay_*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y())
        );
    ...
    epsilon.xz() =
      (pi*Foam::cos(pi*point.z())/2)
       *(
            8*ay_*Foam::cos(4*pi*point.x())*Foam::cos(2*pi*point.y())
          - 15*ax_*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y())
        );
...
)
{
	...
	//adding and edit for the incompressible form:
	Foam::cos(pi*point.z())*(4*ax_*Foam::cos(4*pi*point.x())*Foam::sin(2*pi*point.y())
				 + 2*ay_*Foam::sin(4*pi*point.x())*Foam::cos(2*pi*point.y()) )
    );
}
