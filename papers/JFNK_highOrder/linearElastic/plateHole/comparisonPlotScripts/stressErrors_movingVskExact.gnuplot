...
set output "plateHole_stressErrors_hex_movingVskExact.pdf"
plot \
    ...
    movingFile("hex.struct.ho.N1.summary.txt") u ((area/$4)**0.5*1e3):($6*1e6) w lp ls 112 notitle , \
...
set output "plateHole_stressErrors_tet-unstruct_movingVskExact.pdf"
plot \
    (2e-4 * x**1) w l ls 1 title "1^{st} order", \
...
set output "plateHole_stressErrors_tet-struct_movingVskExact.pdf"
plot \
    (2e-4 * x**1) w l ls 1 title "1^{st} order", \
    (1e-6 * x**2) w l ls 2 title "2^{nd} order", \
    (5e-9 * x**3) w l ls 3 title "3^{rd} order", \
