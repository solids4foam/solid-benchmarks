...
plot \
    ...
    "hex.struct.hypre-snes.summary.txt" u ((volume/$4)**(1.0/3.0)*1e3):($7*1e-6) w lp ps 0.5 pt 5 lc "slategrey" t "L_2 - S4F*", \
    "hex.struct.hypre-snes.summary.txt" u ((volume/$4)**(1.0/3.0)*1e3):($8*1e-6) w lp ps 0.5 pt 4 lc "slategrey" t "L_∞ - S4F*", \
    "hex.struct.seg.summary.txt" u ((volume/$4)**(1.0/3.0)*1e3):($7*1e-6) w lp ps 0.5 pt 5 lc "black" t "L_2 - S4F", \
    "hex.struct.seg.summary.txt" u ((volume/$4)**(1.0/3.0)*1e3):($8*1e-6) w lp ps 0.5 pt 4 lc  "black" t "L_∞ - S4F", \
...
plot \
    ...
    (15e-4 * x**2) w l lw 2 dt 2 lc "blue" notitle,\
    (1e-5 * x**3) w l lw 2 dt 2 lc "violet" notitle,\
    "tet.struct.hypre-snes.summary.txt" u ((4*(volume/$4)/3**0.5)**0.5*1e3):($7*1e-6) w lp ps 0.5 pt 7 lc "slategrey" t "L_2 - S4F*", \
    "tet.struct.hypre-snes.summary.txt" u ((8.48528*(volume/$4))**(1.0/3.0)*1e3):($8*1e-6) w lp ps 0.5 pt 6 lc "slategrey" t "L_∞ - S4F*", \
    "tet.struct.seg.summary.txt" u ((8.48528*(volume/$4))**(1.0/3.0)*1e3):($7*1e-6) w lp ps 0.5 pt 7 lc "black" t "L_2 - S4F", \
    "tet.struct.seg.summary.txt" u ((8.48528*(volume/$4))**(1.0/3.0)*1e3):($8*1e-6) w lp ps 0.5 pt 6 lc "black" t "L_∞ - S4F", \
