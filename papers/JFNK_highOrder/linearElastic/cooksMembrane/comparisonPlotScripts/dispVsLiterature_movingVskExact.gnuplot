...
if (ARGC < 1) {
    print "Usage: gnuplot -c dispVsLiterature_movingVskExact.gnuplot <movingLeastSquares-results-directory>"
    exit
}
...
set output "CooksMembrane-tet-hookeanTipDispConvergence_movingVskExact.pdf"
plot \
    ...
    movingFile("tet.ho.N1.summary.txt") u ((4*(area/$4)/3**0.5)**0.5):($5*1000*1000) w lp ls 111 notitle , \
    movingFile("tet.ho.N2.summary.txt") u ((4*(area/$4)/3**0.5)**0.5):($5*1000*1000) w lp ls 121 notitle , \
    movingFile("tet.ho.N3.summary.txt") u ((4*(area/$4)/3**0.5)**0.5):($5*1000*1000) w lp ls 131 notitle
