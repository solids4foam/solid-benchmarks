...
void Foam::manufacturedSolution::calcBodyForces() const
{
    ...
    if (highOrderIntegration_)
    {
        ...
        forAll (cells, cellI)
        {
            ...
            {
                // Handle arbitary cell by decomposing it into tets
                ...
                forAll(cellTets, tetI)
                {
                    ...
                    const tetPointRef subTetRef = tetIs.tet(mesh);
                    const tetPoints subTet(subTetRef);

                    const tetQuadrature tq(subTet, N);
                    const List<point>& tetQP = tq.points();
                    const List<scalar>& tetQW = tq.weights();

                    const scalar scaleW = tetIs.tet(mesh).mag() / cellV;
                    forAll(tetQP, i)
                    {
                        const vector& quadPoint = tetQP[i];
                        const scalar& weight = tetQW[i];
...
)
{
    ...
    epsilon.xy() =
        ax_*pi*Foam::cos(2*pi*point.y())*Foam::sin(4*pi*point.x())*Foam::sin(pi*point.z())
      + 2*ay_*pi*Foam::cos(4*pi*point.x())*Foam::sin(2*pi*point.y())*Foam::sin(pi*point.z());

    // epsilon.yx() = epsilon.xy();

    epsilon.yz() =
        (ay_*pi*Foam::cos(pi*point.z())*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y()))/2
       + az_*pi*Foam::cos(2*pi*point.y())*Foam::sin(4*pi*point.x())*Foam::sin(pi*point.z());

    // epsilon.zy() = epsilon.yz();

    epsilon.xz() =
        (ax_*pi*Foam::cos(pi*point.z())*Foam::sin(4*pi*point.x())*Foam::sin(2*pi*point.y()))/2
      + 2*az_*pi*Foam::cos(4*pi*point.x())*Foam::sin(2*pi*point.y())*Foam::sin(pi*point.z());
